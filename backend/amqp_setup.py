...
exchangename = "auth_direct" # exchange name
exchangetype = "direct" 

# Instead of hardcoding the values, we can also get them from the environ as shown below
hostname = environ.get('RABBITMQ_HOST', 'localhost') #localhost
port = environ.get('RABBITMQ_PORT', 5672)         #5672 
username = environ.get('RABBITMQ_DEFAULT_USER', 'guest')
password = environ.get('RABBITMQ_DEFAULT_PASS', 'guest')
...
connection_parameters = pika.ConnectionParameters(
    host=hostname, port=port, credentials=credentials, heartbeat=3600, blocked_connection_timeout=3600
)
...
def create_connection(max_retries=12, retry_interval=5):
    ...
    while retries < max_retries:
        try:
            ...
            connection = pika.BlockingConnection(connection_parameters) # these parameters to prolong the expiration time (in seconds) of the connection
...
def create_queues(channel):
    ...
    create_profile_queue(channel)

# function to create Error queue
def create_profile_queue(channel):
    print('amqp_setup:create_profile_queue')
    e_queue_name = 'Profile'
    channel.queue_declare(queue=e_queue_name, durable=True) # 'durable' makes the queue survive broker restarts
    #bind Error queue
    channel.queue_bind(exchange=exchangename, queue=e_queue_name, routing_key='profile')
